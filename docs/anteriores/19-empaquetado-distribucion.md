...
**RA 4. Distribuye aplicaciones multiplataforma**, perteneciente al currículo oficial del Ciclo Formativo de Grado Superior en Desarrollo de Aplicaciones Multiplataforma (DAM) en Andalucía, para el módulo profesional 0488 Desarrollo de Interfaces.

Criterios de evaluación oficiales asociados:

- **CE 4.a)** Se han identificado los diferentes formatos y canales de distribución de aplicaciones de escritorio.
- **CE 4.b)** Se han configurado herramientas de empaquetado y distribución (electron-builder).
- **CE 4.c)** Se ha generado el instalador de la aplicación para diferentes plataformas.
- **CE 4.d)** Se ha configurado un sistema de actualizaciones automáticas.
- **CE 4.e)** Se han aplicado técnicas de firma de código para garantizar la autenticidad del software.
- **CE 4.f)** Se ha automatizado el proceso de construcción y despliegue mediante herramientas de integración continua (CI/CD).
- **CE 4.g)** Se ha verificado el correcto funcionamiento de la aplicación en las diferentes plataformas.

## Conocimientos previos

- **Electron**: comprensión de la arquitectura de procesos (Main Process, Renderer Process), configuración de `BrowserWindow`, scripts de precarga (preload.js), IPC (Inter-Process Communication), y manejo de APIs nativas (estudiados en la unidad 18).
...
1. Partiendo del proyecto "Gestor de Notas" (Angular + Electron) desarrollado en la unidad 18, instala electron-builder: `npm install --save-dev electron-builder`.
