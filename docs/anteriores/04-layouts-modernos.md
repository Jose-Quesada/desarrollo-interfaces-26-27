...
Esta unidad contribuye de manera directa y sustancial a este resultado de aprendizaje, concretándose en los siguientes criterios de evaluación del currículo oficial:

- CE a. Se han reconocido los elementos de una interfaz web (botones, formularios, tablas, menús, tarjetas, modales, layouts).
- CE c. Se ha generado un sistema de diseño con elementos reutilizables (layouts y patrones de composición).
- CE d. Se han aplicado los principios de diseño responsive (mobile-first, breakpoints, container queries).
- CE e. Se han utilizado herramientas de desarrollo (Tailwind CSS, DevTools) para generar interfaces.

Asimismo, esta unidad está estrechamente vinculada al **RA 3** ("Implementa interfaces web utilizando lenguajes de marcas y hojas de estilo") en sus criterios:

- CE a. Se han utilizado lenguajes de marcas para estructurar el contenido (HTML5 semántico en las templates de Angular).
- CE b. Se han aplicado hojas de estilo en cascada para el diseño visual (Tailwind CSS aplicando utilidades de layout).
...
- Tailwind CSS nivel básico (cubierto en la Unidad 2): comprensión del enfoque utility-first, capacidad de aplicar clases de Tailwind en templates HTML, familiaridad con el sistema de colores, tipografías y espaciado.
- Angular nivel básico (Unidad 2): capacidad de crear componentes standalone con Angular CLI, comprensión de la estructura de un componente (template, clase TypeScript, estilos), manejo de inputs y control flow (@if, @for).
- Diseño en Figma (Unidad 3): capacidad de interpretar un diseño en Figma y extraer medidas, colores, tipografías y estructuras de layout (Auto Layout → Flexbox).
...
2. Traducir mentalmente las estructuras de Auto Layout de Figma a Flexbox/Grid de Tailwind (recordando la equivalencia de la Unidad 3: Auto Layout horizontal = flex-row, Auto Layout vertical = flex-col).
...
En la Unidad 5, la última de este módulo, cerraremos el ciclo completo: tomaremos diseños de Figma, extraeremos sus design tokens, configuraremos nuestro tema de Tailwind, implementaremos los componentes en Angular, los documentaremos en Storybook, y construiremos pantallas completas. Todo lo aprendido en Flexbox, Grid y posicionamiento será la base sobre la que se asentarán esas implementaciones.
